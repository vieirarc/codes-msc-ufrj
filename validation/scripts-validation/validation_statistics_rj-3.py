...
for index, name in enumerate(datesList):
	...
	tpPointResults = tpField[119, 154]
...
for k, v in hsBuoyFinalDictOrdered.items():
	sumHsXi = sumHsXi + v
Rj3HsXm = (1/len(hsBuoyFinalDictOrdered))*sumHsXi
...
for k, v in tpBuoyFinalDictOrdered.items():
	sumTpXi = sumTpXi + v
Rj3TpXm = (1/len(tpBuoyFinalDictOrdered))*sumTpXi
...
# Scatter Index HS
SI_Hs = RMSE_Hs/Rj3HsXm

# Scatter Index TP
SI_Tp = RMSE_Tp/Rj3TpXm
...
f.write('__________________________________________________')
f.write('\n')
f.write('Mean Tp Buoy: ' + str(Rj3TpXm))
f.write('\n')
