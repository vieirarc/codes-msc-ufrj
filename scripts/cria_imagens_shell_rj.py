...
# Pastas e arquivo
datasetHsRegional = Dataset(resultsDir + simulationName + '/rj/ww3.' + startingDate[0:6] + '_hs.nc')
datasetDirRegional = Dataset(resultsDir + simulationName + '/rj/ww3.' + startingDate[0:6] + '_dir.nc')
pathSaveRegional = (resultsDir + simulationName + '/rj/imagens')
...
for index, name in enumerate(listField):
    m = Basemap(llcrnrlon=-45.5, llcrnrlat=-26.5, urcrnrlon=-40.5, urcrnrlat=-21.5, resolution='i', projection='merc')
    m.drawcoastlines(linewidth=0.8)
    m.drawstates(linewidth=0.1)
    m.drawcountries(linewidth=0.6)
    m.fillcontinents(color='0.8', lake_color=None)
    m.drawparallels(np.arange(-26, -21 ,2), fontsize=7, linewidth=0.03, labels=[1,0,0,0])
    m.drawmeridians(np.arange(m.lonmin,m.lonmax, 2), fontsize=7, linewidth=0.03, labels=[0,0,0,1])
    waveFieldHsRegional = waveHsRegional[index,:,:]
    u, v = intdir2uv(1, waveDirRegional[index,:,:])
    ut = -u
    vt = -v
    x, y = m(lonCorr,lat)
    im = m.pcolormesh(x, y,waveFieldHsRegional, vmin=0, vmax=4.1, shading='gouraud')  
    ...
    cbar.set_ticks([np.arange(0, 4.1, 0.25)], update_ticks=True)
